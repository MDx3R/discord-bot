...
api = ''
header = {'accept': 'application/json', 'Authorization': 'Bearer {}'.format(api)}
...
def faceit_get_player_nickname(player_id):
    response = requests.get(f'{url}/players/{player_id}', headers=header)
...
def faceit_get_player_data(nickname):
    query_params = {'nickname': nickname}
    response = requests.get(url + '/players', headers=header, params=query_params)
...
def faceit_get_match_details(match_id):
    query_params = {'match_id': match_id}
    response = requests.get(url + '/matches/{}'.format(match_id), headers=header, params=query_params)
